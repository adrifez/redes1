...
awk -v ip="$(wc -l a1/ip.dat)" '{noip=noip+1;} END{total=ip+noip; print (ip*100)/total; print (noip*100/total);}' a1/noip.dat
echo Del porcentaje de IP tenemos: TCP, UDP y otros respectivamente
awk -v ip="$(wc -l a1/ip.dat)" '{if($1 == 6) tcp=tcp+1; else if($1 == 17) udp=udp+1; else otros=otros+1;} END{print (tcp*100)/ip; print (udp*100)/ip; print (otros*100)/ip;}' a1/ip.dat
